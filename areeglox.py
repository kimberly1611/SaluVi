...
class Ventana1:
    def __init__(self, root):
       ...
       # Cargar imagen con ruta completa
        imagen = Image.open("C:/Users/damia/OneDrive/Escritorio/proyecto_final/saluuviid.png")
...
class Ventana2:
    ...
    def ir_a_ventana3(self):
        self.root.withdraw()
        Ventana3(tk.Toplevel(), ventana_anterior=self.root)

    def iniciar_sesion(self):
        self.root.withdraw()
        VentanaLogin(tk.Toplevel(), ventana_anterior=self.root)

    def ir_a_quizas_mas_tarde(self):
        self.root.withdraw()
        VentanaQuizasMasTarde(tk.Toplevel(), ventana_anterior=self.root)
...
class Ventana3():
    ...
    def calcular_imc(self):
        try:
            ...
            self.root.withdraw()
            Ventana4(tk.Toplevel(), imc, self.sexo.get(), self.edad.get(), ventana_anterior=self.root, desde_registro=True, ventana3=self.root)
        except ValueError:
...
class Ventana4:
    ...
    def ir_a_ventana5(self, imc, genero, edad):
        self.root.withdraw()
        Ventana5(tk.Toplevel(), imc, genero, edad, ventana_anterior=self.ventana_anterior, ventana3=self.ventana3)

class Ventana5:
    def __init__(self, root, imc, genero, edad, ventana_anterior=None, ventana3=None, from_login=False):
        self.root = root
        ...
        self.ventana3 = ventana3
        self.ventana_anterior = ventana_anterior
        self.from_login = from_login  # Para saber si regresar a Ventana2

        tk.Label(root, text="¡Con qué deseas comenzar!", font=("Copperplate Gothic Bold", 16), bg="#CCCCCB").pack(pady=43)

        tk.Button(root, text="Dieta", font=("Engravers MT", 13), width=20, bg="#E2BF92", command=self.ir_a_dieta).pack(pady=43)
        tk.Button(root, text="Ejercicio", font=("Engravers MT", 13), width=20, bg="#E2BF92", command=self.ir_a_ejercicio).pack(pady=43)
        tk.Button(root, text="Regresar", font=("Engravers MT", 13), width=20, bg="#E2BF92", command=self.regresar).pack(pady=43)
    ...
    def ir_a_ejercicio(self):
        self.root.withdraw()
        VentanaEjercicio(tk.Toplevel(), self.root, self.imc, self.genero, self.edad, from_login=True)  # o False, según el caso
...
Cena: Puré de zanahoria con tofu."""
    ...
    def regresar(self):
        self.root.destroy()
        if self.from_login:
            # Reactiva la ventana anterior (Ventana5), que fue pasada como ventana_anterior
            self.ventana_anterior.deiconify()
        else:
            self.ventana_anterior.deiconify()
...
class VentanaLogin:
    ...
    def verificar(self):
        ...
        if datos:
            self.root.withdraw()
            Ventana5(tk.Toplevel(), datos["imc"], datos["sexo"], datos["edad"], ventana_anterior=self.root, from_login=True)
        else:
